...
| `epic_members` | `(&StateGraph, &[(StateSource, StateFile)], &str) -> Vec<(String, &TrackBlock)>` | Every block claiming a slug, across all repos, in `wave_order` — the cross-repo sequence for one initiative. Ordering is delegated to `wave_order` rather than reimplemented, so a per-epic table and a per-repo wave table can never disagree about sequence. |
