...
`emit_state(root: &Path, write: bool) -> anyhow::Result<Report>` (in `src/lib.rs`) resolves `brain.toml`, discovers and loads all state files, builds the graph, then runs all five planners — `plan_state_json`, `plan_master_plan_tables`, `plan_project_caches`, `plan_tier_rollups`, `plan_hq_board` — in that stable order, applies each plan with `apply_plan(write)`, and merges all diagnostics into a single `Report`. Invoked by `mev emit-state`. MV.4.E wired the last three planners in; prior to that they existed but were not reachable from `emit_state`.
