...
├── shared.rs       ← internal helpers: extract_frontmatter, is_kebab_case, non_empty, and `git_command()` — **every `git` subprocess in this crate MUST go through `git_command()` (or, from `tests/`, `testsupport::git_command()`), never `Command::new("git")`.** Git exports `GIT_DIR` (and `GIT_INDEX_FILE`, `GIT_WORK_TREE`, `GIT_OBJECT_DIRECTORY`, …) to every hook it runs, and those variables **override both `-C` and `current_dir`** — so a call meant for one path silently operates on the hook's repository instead. `git_command()` strips the nine repository-scoping variables listed in `GIT_REPO_ENV_VARS`. This is not hypothetical: because `hooks/pre-push` stage 2 runs `cargo test`, mev's own suite was structurally unable to pass from inside a push (ten failures across `brain::config::is_linked_worktree`, `consumers::tests` and `brain_emit`'s worktree guard, all passing from a shell), and a `brain_emit` fixture ran `git init` / `git commit` / `git worktree add` **against the real `core/mev` repository**, moving `main` onto a junk commit. Fixed 2026-08-21 (`MV.17.A` lane hotfix); the production callers that had the same defect were `consumers::git_is_dirty` and `brain::config::run_git_rev_parse`, which is why pre-push stage 3 reported sibling repos as NOT-EVALUABLE with `git status` exiting 128
...
└── brain/
    ...
    └── lane_segments.rs ← Reads the authored `lane-<name>.json` record — not the old line-oriented `.txt` directive grammar, retired by `MV.17.A` Task 2. Schema owner: `base-template/.claude/workflows/lane.schema.json` (D71); this module's on-disk shape (`LaneRecord`/`LaneRecordBlock`) is `#[serde(deny_unknown_fields)]`, so an unrecognised top-level or block-level key is a loud `E_LANE_RECORD_MALFORMED` diagnostic, never a silent skip. Each record is an ordered `blocks[]` array; every block entry carries its own authored `repo` and `origin_roadmap` (`LaneBlockRef`), so there is no comment syntax to mis-parse and no unannotated cross-roadmap claim left to arbitrate. `MV.20.C` Task 1 added an optional `lease_windows: [{repo, blocks}]` (`LeaseWindow`, also `deny_unknown_fields`) as a sibling field on `LaneDirectives`/`LaneRecord`, alongside — never replacing — the existing whole-lane `exclusive_repos`: `exclusive_repos` still means exclusivity for the lane's whole chain, while a `lease_windows` entry narrows that grant to only the block ids it names. A window naming a block id absent from that same record's own `blocks[]` is reported as a non-fatal `E_LANE_LEASE_WINDOW_UNKNOWN_BLOCK` diagnostic (record still parses and is still returned) rather than a hard parse failure. Task 2 mirrored the shape into the schema owner, `base-template/.claude/workflows/lane.schema.json`, and into `base-template/scripts/check_lane_records.py`'s validation, with a live-corpus parity test (`live_corpus_lane_schema_agrees_with_mev_parser_about_lease_windows`) pinning the two against drift. — **`E_LANE_DIRECTIVE_UNRECOGNISED` and the double-claim error are retired as unrepresentable states in this format, not merely fixed.** `discover_lane_files()` walks both roadmap layouts — the current `planning/roadmaps/<slug>/` and the legacy `planning/<slug>/` — for `lane-<name>.json` files (glob owned by `is_lane_file_name()`/`lane_name_from_file()`); a slug present under both locations at once is still reported as ambiguous rather than silently preferred. New: `warn_if_roadmap_dir_has_no_lane_record()` emits `W_LANE_DIR_NO_RECORD` when a roadmap directory (one with a top-level `roadmap.md`) has no `lane-*.json` record at all, so a roadmap whose lane file is missing or misnamed surfaces instead of just deriving zero positions. Downstream of discovery, the derived-output half is unchanged and byte-compatible: `LaneSegment`, `LaneSegmentBlock` (with `.head()`), `build_owner_index()`, `resolve_owner()` (`None` on zero or >1 matching repo — never guesses), `segment_lane_blocks()`/`segment_lane_file()` (cuts a new `(repo, chain)` segment on every ownership change), `unresolved_owner_diagnostics()`, `derive_lane_positions()` (final `{roadmap, lane, segment, position}` plus authored `origin_roadmap` per block), `plan_lane_segments()` (`emit-state` planner writing `planning/lane-segments.json`; written in full on an unscoped run, but as of `MV.ticket.emit-state-write-is-corpus-wide-and-unscoped` task 2 its plan is now passed through `filter_plan_by_scope` too — since none of its target paths match a scope's four per-repo surfaces, a `--scope <repo>` run writes none of it, matching the frontier/availability planners below), `apply_with_rollback_on_regression()`, `roadmap_slug_from_plan_path()`, `derive_program_membership()` (feeds `emit::epic_members_resolved`'s `kind: program` case), and the frozen cross-repo contract types `LaneDirectives`/`LaneBudget`/`DerivedBlockPosition`/`LaneSegmentsArtifact` — `core/engine-rs`'s `chain.rs` re-declares `LaneDirectives`/`LaneBudget` as `deny_unknown_fields` mirrors and `LANE_SEGMENTS_ARTIFACT` is a frozen cross-repo contract, so none of their field sets changed here. `deferred-blocks.txt` stays out of scope on purpose — it is the only place an operator-cut block is encoded, and the `lane-<name>.json` glob must never widen to catch it. MV.13.A (cross-repo lane-file segmentation); MV.13.D Task 3 added `roadmap_slug_from_plan_path()`/`derive_program_membership()`; `MV.17.A` (D71) replaced the authored-input half for `lane.json` while holding the derived half frozen. **Converting the existing corpus's `.txt` lane files to `lane-<name>.json` and installing the updated binary belong to `HQ.8.A`, not this module.**
└── doc/
...
├── emit_state_scope.rs ← integration tests for `emit-state --scope` (byte-identity of unvisited repos, unknown-slug diagnostic, unscoped-unchanged) — ticket-emit-state-scope-and-lock; extended by `MV.ticket.emit-state-write-is-corpus-wide-and-unscoped` with `scoped_write_touches_no_file_outside_scope` (whole-tree walk pinning that a scoped write leaves every file outside the four target surfaces, including the three corpus-wide artifacts, byte-identical) and `unscoped_write_still_writes_lane_derived_artifacts` (control: an unscoped run still writes lane-segments/frontier/availability in full)
...
├── emit_state_authored_roundtrip.rs ← integration tests pinning that authored fields on `reference[]`/`carryover[]` entries survive an `emit-state --write` round-trip unchanged at the field level (Test A: full JSON-value diff), plus a control (Test B) showing a container-count/slug-set comparison alone is blind to the same mutation — `MV.ticket.emit-state-write-is-corpus-wide-and-unscoped`
...
| `check_backlog_integrity` | `(&[(StateSource, StateFile)], &StateGraph) -> Vec<Diagnostic>` | Backlog-node integrity: dangling `depends_on` targets (`E_STATE_DANGLING_BLOCKED_BY`) and orphan/unresolved promoted nodes (`E_STATE_DANGLING_PROMOTION`). Operator option-(b) decision (`MV.ticket.demote-block-leaves-inbound-refs-dangling`): a `parked` backlog[] entry is a legitimate resolution target for a block reference, so a reference to a parked block is not dangling — and a parked entry's own snapshot `depends_on` is not validated as a live edge. |
| `ready_order` | `(&StateGraph, &[(StateSource, StateFile)], gating: Option<&BTreeMap<String, RepoGatingReport>>) -> Vec<String>` | Reusable standalone function: returns open blocks ordered by `wave` (tiebreak: track order, array order) whose every `type:block` dep is `closed` and which have no `type:external` deps. Forward-compat input for `MV.3B.T` topo-emit. `gating` (`MV.16.C`) is the per-repo carryover-enforcement gating set from `carryover::build_carryover_gating_sets`, keyed by the target block's repo; `None` (or a repo absent from the map) means "no carryover gate applies" — exactly today's behaviour. A block held by a gate is excluded from the ready set even though its authored `depends_on` is fully met. All production call sites pass `None` for now (see `build_carryover_gating_sets` below). |
...
| `derive_focus` | `(&StateSource, &StateFile, &StateGraph, &[(StateSource, StateFile)], gating: Option<&BTreeMap<String, RepoGatingReport>>) -> DerivedFocus` | **Single-source derivation** — computes the expected `focus` from authored `tracks[]`: `now` = `in_progress` blocks; `next` = ready open blocks in `wave` order; `blocked` = open blocks with an unmet `depends_on`, each carrying the unmet subset as `blocked_by[]`. Called by both `check_focus_drift` and the emit planners. `gating` (`MV.16.C`, same shape as `ready_order`'s) additionally holds an `open` block in `blocked` — with a `DerivedFocus.carryover_gates` entry naming the owning slug — even when it has no unmet `depends_on` of its own: the invisibility case a purely-carryover-gated block would otherwise fall through. |
...
| `DerivedFocus` | Output of `derive_focus`: `now: Vec<String>`, `next: Vec<String>`, `blocked: Vec<(String, Vec<BlockedBy>)>` — block ids (and, for `blocked`, the unmet subset of each block's `depends_on`) — plus `carryover_gates: BTreeMap<String, Vec<CarryoverGate>>` (`MV.16.C`), mapping a gated block id to the gate(s) holding it. |
...
- **`compute_frontier(lane_positions, graph, files, effective, gating) -> Frontier`** — walks
