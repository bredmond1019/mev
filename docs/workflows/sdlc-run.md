...
| **Implement** | sonnet | Executes the task(s) against the spec (and `breakdown.md` if present). Runs a completeness self-check (D8) before committing `feat:`/`fix:`. |
| **Test** | haiku | Runs the project's validation suite from `harness.json` (`validation.checks[]`, incl. richer D6/D26 check kinds) + the universal emoji gate. Reads exit codes; writes `test.md`. |
...
| **Wrap-up** | sonnet | Updates `status.md` + appends the `log.md` entry + writes `workflow.md`, then one `chore:` commit. Merged log-work + finalize into one agent (D14); applies any spec Amendment-Log deviations on `main` (D18). |
...
and the token-accounting artifact. See D37.
