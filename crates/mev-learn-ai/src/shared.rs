//! Shared helper functions used by this crate's validators. Mirrors the subset of
//! `mev::shared` that `mev-learn-ai` actually uses — string-manipulation and format-checking
//! utilities that carry no domain types. `mev-learn-ai` cannot depend on `mev` (see the crate
//! doc comment in `lib.rs`), so these are mirrored here rather than imported.
