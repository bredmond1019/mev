...
**Current focus:** `MV.4.E` (spine terminus, wires `MV.4.B`+`MV.4.C` planners into `emit_state`) — Phase 4, state-sync-loop. See `core/planning/state-sync-loop/master-plan.md` for details. Prior: `4.D-sync-comparator-hardening` (Done), `4.C-hq-board-emit` (Done), `4.B-cache-rollup-emit` (Done), `4.A-emit-foundation` (Done).
...
- **now** — **`4.D-sync-comparator-hardening` Done** (Phase 4, state-sync-loop Wave 3; full spec, PASS). `check_sync`'s watermark comparison (`src/brain/sync.rs`) hardened to compare explicit UTC instants (`.to_utc()`), with a doc comment stating the invariant and 2 new offset-mismatch regression tests. **Prior:** `4.C-hq-board-emit` (Phase 4, Wave 2; 3 tasks, PASS); `4.B-cache-rollup-emit` (Phase 4, Wave 2; 3 tasks, PASS); `4.A-emit-foundation` (Phase 4, Wave 1; 4 tasks, PASS).
- **next** — `MV.4.E` (spine terminus, wires `MV.4.B`+`MV.4.C` planners into `emit_state`) per `core/planning/state-sync-loop/master-plan.md` — now the sole remaining Phase 4 block; or the out-of-repo orchestrator follow-up (`load_brain_edges.py` deletes its own resolution logic and reads mev's exported fields).
