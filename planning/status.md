...
**Current focus:** `4.D-sync-comparator-hardening` In progress — Phase 4, state-sync-loop Wave 3. See `core/planning/state-sync-loop/master-plan.md` and `planning/4.D/spec.md` for details. Prior: `4.C-hq-board-emit` (Done), `4.B-cache-rollup-emit` (Done), `4.A-emit-foundation` (Done).
...
| MV.4.D | Sync-comparator hardening | In progress | Spec definition and task breakdown in progress. See `planning/4.D/spec.md`. |
