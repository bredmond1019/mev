...
**Current focus:** Phase 4 (state-sync-loop) is fully closed — `4.E-emit-state-wiring` (spine terminus) shipped PASS, wiring `MV.4.B`+`MV.4.C` planners into `emit_state`. See `core/planning/state-sync-loop/master-plan.md` for the full spine history. Next: pick the next phase/spec per `master-plan.md`.
...
- **now** — **`4.E-emit-state-wiring` Done** (Phase 4, state-sync-loop spine terminus; full spec, 4 tasks, PASS). `emit_state` now wires all five planners (`plan_state_json`, `plan_master_plan_tables`, `plan_project_caches`, `plan_tier_rollups`, `plan_hq_board`) via `apply_plan`; new `mv4e_ripple` integration test proves a single call ripples a cross-repo dependency change across every surface plus a fixed-point guarantee. This closes Phase 4 entirely. **Prior:** `4.D-sync-comparator-hardening` (Phase 4, Wave 3; PASS); `4.C-hq-board-emit` (Phase 4, Wave 2; 3 tasks, PASS); `4.B-cache-rollup-emit` (Phase 4, Wave 2; 3 tasks, PASS); `4.A-emit-foundation` (Phase 4, Wave 1; 4 tasks, PASS).
- **next** — Phase 4 (state-sync-loop) is fully closed; pick the next phase/spec per `master-plan.md`. Also outstanding: the out-of-repo orchestrator follow-up (`load_brain_edges.py` deletes its own resolution logic and reads mev's exported fields).
...
| MV.4.D | Sync-comparator hardening | Done | Full spec, PASS. `check_sync`'s watermark comparison (`src/brain/sync.rs`) hardened to `source_dt.to_utc() != cache_dt.to_utc()` with a doc comment stating the instant-comparison invariant; 2 new regression tests (`same_instant_across_offsets_produces_no_e_sync_drift`, `different_instant_across_offsets_produces_e_sync_drift`); all four harness gates green (314 tests); `docs/cli.md` `--sync` section patched to describe instant-based comparison. |
| MV.4.E | Emit-state wiring (spine terminus) | Done | 4 tasks, PASS. `emit_state` (`src/lib.rs`) wires all five planners — `plan_state_json`, `plan_master_plan_tables`, `plan_project_caches`, `plan_tier_rollups`, `plan_hq_board` — each applied via `apply_plan` in stable order; doc comment names all five surfaces. New `mv4e_ripple` integration test (`tests/brain_emit.rs`) proves a single `emit_state` call ripples a close-A-unblocks-B cross-repo change across every surface (leaf focus, project cache + `synced_from`, tier rollup, HQ board, master-plan wave table) plus a fixed-point check. `docs/cli.md` updated. All four harness gates green. Closes Phase 4 (state-sync-loop) spine: `MV.4.A → {B,C,D} → E` fully Done. |
