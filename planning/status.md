...
**Current focus:** No open mev-tracked work. `update-write-state-in-trees` (the last open ticket) shipped PASS and closed; Phase 4 (state-sync-loop) is fully closed — see `core/planning/state-sync-loop/master-plan.md` for the full spine history. Next: pick the next phase/spec per `master-plan.md`, or the out-of-repo orchestrator follow-up (`load_brain_edges.py`).
...
- **now** — **`update-write-state-in-trees` Done** (ticket; full spec, 5 tasks, PASS, run in place on `main`). `is_linked_worktree()` added to `src/brain/config.rs`; `emit-state --write` now refuses (`E_EMIT_LINKED_WORKTREE`, non-zero exit) when invoked from a linked git worktree instead of silently clobbering the main checkout's derived files; dry-run unaffected. PR #18 (`4.E-emit-state-wiring`) merged; `emit-state --write` run from main to regenerate brain-wide derived views. `focus` is now empty. **Prior:** `4.E-emit-state-wiring` (Phase 4 spine terminus; 4 tasks, PASS) closed Phase 4 entirely; `4.D-sync-comparator-hardening` (Phase 4, Wave 3; PASS); `4.C-hq-board-emit` (Phase 4, Wave 2; 3 tasks, PASS); `4.B-cache-rollup-emit` (Phase 4, Wave 2; 3 tasks, PASS); `4.A-emit-foundation` (Phase 4, Wave 1; 4 tasks, PASS).
- **next** — No open mev-tracked work; pick the next phase/spec per `master-plan.md`. Also outstanding: the out-of-repo orchestrator follow-up (`load_brain_edges.py` deletes its own resolution logic and reads mev's exported fields).
