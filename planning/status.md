...
- **now** — **`4.B-cache-rollup-emit` Done** (Phase 4, state-sync-loop Wave 2; 3 tasks, PASS). `plan_project_caches` (focus-line + `synced_from` splice into `docs/projects/<slug>.md`) and `plan_tier_rollups` (`derive_rollup` rows spliced into each tier's `status.md`) are landed in `src/brain/emit.rs`, both following `plan_master_plan_tables`'s splice/fixed-point/`W_EMIT_NO_SENTINEL` pattern; neither wired into `emit_state` yet. **Prior:** `4.A-emit-foundation` (Phase 4, Wave 1; 4 tasks, PASS).
- **next** — `MV.4.C` (HQ board) or `MV.4.E` (spine terminus, wires `MV.4.B`+`MV.4.C` planners into `emit_state`) per `core/planning/state-sync-loop/master-plan.md`, or the out-of-repo orchestrator follow-up (`load_brain_edges.py` deletes its own resolution logic and reads mev's exported fields).
...
| MV.4.E | (spine terminus) | Not started | Depends on MV.4.B (done)/MV.4.C. |
