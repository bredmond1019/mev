...
// BT.ticket.prepare-run-replaces-setup-agents (task 6, AC4): a refused prepare_run.py verdict
// stops the run HERE — before any worktree is created and before any implement-stage agent ever
// launches — surfacing prepare_run.py's own `reason` exactly as today's setup agents would report
// an unrunnable spec.
...
await snapshotBaselines(harnessCfg, runDir, baseSha)
...
async function runTests(label, { gatingOnly, taskCommands = null, expectRedSet = new Set(), onPass = null, engineFiles = [] }) {
  ...
  if (usingOverride) {
    ...
    checklistBody = renderCheckList(harnessCfg, { gatingOnly, cwd: runDir, engineFiles, baseSha })
...
`, withModel({ label: `${isFix ? 'fix' : 'implement'}-${taskNum}-${attempt}`, schema: STAGE_SCHEMA, phase: 'Tasks' }, isFix ? fixModel : MODEL.implement))
    ...
    if (failingCheckIds.length === 1) {
      attribution = await attributionLookback({ checkId: failingCheckIds[0], state, taskNum, runRoot: runDir, baseSha, harnessCfg, GIT })
      if (attribution) {
...
${renderCheckList(reconcileCfg, { gatingOnly: false, cwd: runDir, engineFiles: [], baseSha })}
